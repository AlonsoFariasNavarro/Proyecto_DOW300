...
@section('if-nombre')
    @if(Route::current()->getName()=='admin.edit')
        ...
        <div class="row">
            ...
            <div class="col d-flex flex-column py-5">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{route('profesor.update',$profesor->rut)}}" method="POST">
                            @method('PUT')
                            @csrf
                            <div>
                                <label for="rut">RUT</label>
                                <input for="rut" id="rut" name="rut" class="form-control" type="text" value="{{$profesor->rut}}">
                            </div>
                            <div>
                                <label for="nombre">Nombre</label>
                                <input for="nombre" id="nombre" name="nombre" class="form-control" type="text" value="{{$profesor->nombre}}">
                            </div>
                            <div>
                                <label for="apellido">Apellido</label>
                                <input for="apellido" id="apellido" name="apellido" class="form-control" type="text" value="{{$profesor->apellido}}">
                            </div>
